...
class LyricFolderPreference(widgets.PrefsItem):
    default = '~/lyrics'
    name = 'plugin/LyricDisp/lf'

class LyricColor(widgets.ColorButtonPrefsItem):
    def _setup_change(self):
        widgets.ColorButtonPrefsItem._setup_change(self)
    def change(self, *args):
        event.log_event('color_change', self, self._get_value())
    default = '#43AAD0'
    name = 'plugin/LyricDisp/lc'

class ModePreference(widgets.ComboPrefsItem):
    default = '窗口模式'
    name = 'plugin/LyricDisp/ms'
    def _setup_change(self):
        widgets.ComboPrefsItem._setup_change(self)
    def change(self, *args):
        event.log_event('mode_change', self, self._get_value())

class LyricNamePreference(widgets.ComboPrefsItem):
